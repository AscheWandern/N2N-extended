...
from .common import default_conv, Scale, Upsampler
...
class Un(nn.Module):
    def __init__(self,n_feats, wn):
        ...
        self.weight2 = Scale(1)
        self.weight1 = Scale(1)
        self.attention = MLABlock(n_feat=n_feats, dim=432) 
...
class ESRT(nn.Module):
    def __init__(self, in_nc, n_feature, conv=default_conv):
        ...
        modules_tail = [
            ...
            conv(n_feats, in_nc, 1)]


        #self.up = nn.Sequential(Upsampler(conv,4,n_feats,act=False),
                          #BasicConv(n_feats, in_nc,3,1,1))
        self.up = nn.Sequential(BasicConv(n_feats, in_nc,3,1,1))
    ...
    def forward(self, x1,x2 = None, test=False):
        ...
        x1 = self.tail(res1)
        #print(x1.shape)
        #print(self.up(res2).shape)
        #print(self.up(res2).shape, x1.shape)
        x1 = self.up(res2) + x1
        # x1 = self.add_mean(x1)
        #x2 = self.tail(res2)
        return x1
