...
from models.util.tools import extract_image_patches,\
    reduce_mean, reduce_sum, same_padding, reverse_patches
from models.util.position import PositionEmbeddingLearned, PositionEmbeddingSine
import pdb
import math
...
class EffAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, 4, C // 4).permute(2, 0, 3, 1, 4)
...
class MLABlock(nn.Module):
    def __init__(
        ...
        self.dim = dim
        self.atten = EffAttention(self.dim, num_heads=4, qkv_bias=False, qk_scale=None, \
